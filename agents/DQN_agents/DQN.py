...
class DQN(Base_Agent):
    ...
    def pick_action2(self, state=None, list=None):
        """Uses the local Q network and an epsilon greedy policy to pick an action"""
        # PyTorch only accepts mini-batches and not single observations so we have to use unsqueeze to add
        # a "fake" dimension to make it a mini-batch rather than a single observation
        if state is None: state = self.state
        ...
        if len(state.shape) < 2: state = state.unsqueeze(0)
        best=np.zeros((5,6))
        for lqind in range(len(list)):
            list[lqind].eval() #puts network in evaluation mode
            with torch.no_grad():
                action_values = list[lqind](state)
            # print("ppp",action_values)
            best[lqind, :]=action_values
        # print("best",best)
        # print("argmax", np.argwhere(best.max()==best))
        nn = len(np.argwhere(best.max()==best))
        # print("nn", nn)
        ii = random.randint(0,nn-1)
        # print("ii", ii)
        indbestlq = np.argwhere(best.max()==best)[ii][0]
        # print("best", np.argwhere(best.max()==best)[ii])
        bestlq = list[indbestlq]

        bestlq.eval()  # puts network in evaluation mode
        with torch.no_grad():
            action_values = bestlq(state)
        bestlq.train()  # puts network back in training mode
        action = self.exploration_strategy.perturb_action_for_exploration_purposes({"action_values": action_values,
                                                                                    "turn_off_exploration": self.turn_off_exploration,
                                                                                    "episode_number": self.episode_number})
        self.logger.info("Q values {} -- Action chosen {}".format(action_values, action))
