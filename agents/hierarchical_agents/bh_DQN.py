...
class bh_DQN(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.controller.q_network_target = self.create_NN(input_dim=self.state_size*2, output_dim=self.action_size, key_to_use="CONTROLLER")
        ...
        self.list_meta_controller = [DDQN(self.meta_controller_config) for _ in range(5)]
        self.lq_network_local = []
        self.lq_network_target = []
        for m in self.list_meta_controller:
            m.q_network_local = self.create_NN(input_dim=self.state_size, output_dim=config.environment.observation_space.n,
                                             key_to_use="META_CONTROLLER")
            self.lq_network_local.append(m.q_network_local)
            m.q_network_target = self.create_NN(input_dim=self.state_size, output_dim=config.environment.observation_space.n,
                                              key_to_use="META_CONTROLLER")
    ...
    def reset_game(self):
        ...
        self.meta_controller = self.list_meta_controller[random.randint(0,4)]
    ...
    def step(self):
        ...
        self.meta_controller = self.list_meta_controller[random.randint(0, 5-1)]

        while not self.episode_over:
            episode_intrinsic_rewards = []
            self.meta_controller_state = self.environment.state
            self.subgoal = self.meta_controller.pick_action2(state=self.meta_controller_state, list=self.lq_network_local)
            # self.subgoal = self.meta_controller.pick_action(state=self.meta_controller_state)
            self.goals_seen.append(self.subgoal)
            self.subgoal_achieved = False
            self.state = np.concatenate((np.array([self.environment.state]), np.array([self.subgoal])))
            self.cumulative_meta_controller_reward = 0
    ...
    def update_controller_data(self):
        ...
        environment_next_state = self.environment.next_state
        environment_next_state = np.array([environment_next_state])
    ...
    def update_meta_controller_data(self):
        ...
        self.cumulative_meta_controller_reward += self.environment.reward
        # self.total_episode_score_so_far += self.environment.get_reward()
        self.total_episode_score_so_far += self.environment.reward
        if self.done:
            # self.meta_controller_next_state = self.environment.get_next_state()
            self.meta_controller_next_state = self.environment.next_state
